...
class AlgoEvent:
    ...
    def on_marketdatafeed(self, md, ab):
        ...
        if pd.isna(sma.iloc[-1]) or pd.isna(trend_sma.iloc[-1]) or pd.isna(std.iloc[-1]) or pd.isna(atr.iloc[-1]):
            return

        latest_close = float(close.iloc[-1])
        latest_sma = float(sma.iloc[-1])
        latest_trend_sma = float(trend_sma.iloc[-1])
        latest_std = float(std.iloc[-1])
        latest_atr = float(atr.iloc[-1])
        if latest_std <= 0:
            return

        # Single-module change in round13: add normalized volatility bandwidth regime filter.
        bandwidth = latest_std / latest_sma if latest_sma != 0 else 0
        if bandwidth < 0.008 or bandwidth > 0.09:
            return

        if latest_close <= latest_trend_sma:
            return
