...
class AlgoEvent:
    ...
    def on_marketdatafeed(self, md, ab):
        ...
        if self.last_entry_day is not None:
            ...
            if days_since_entry < self.cooldown_days:
                return

        count = max(self.slow_sma_period, self.std_period) + self.atr_period + 60
        ...
        sma_fast = close.rolling(self.sma_period).mean()
        sma_slow = close.rolling(self.slow_sma_period).mean()
        std = close.rolling(self.std_period).std()
        atr = self._atr(high, low, close, self.atr_period)

        if pd.isna(sma_fast.iloc[-1]) or pd.isna(sma_slow.iloc[-1]) or pd.isna(std.iloc[-1]) or pd.isna(atr.iloc[-1]):
            return
        ...
        if latest_std <= 0 or latest_sma_fast <= 0:
            return

        # Regime filter: avoid strong trend regimes for mean reversion entries.
        trend_ratio = abs(latest_sma_fast - latest_sma_slow) / latest_sma_slow
        if trend_ratio > 0.08:
            return

        bandwidth = latest_std / latest_sma_fast
        if bandwidth < 0.01 or bandwidth > 0.12:
            return

        zscore = (latest_close - latest_sma_fast) / latest_std
