...
class AlgoEvent:
    """
    Round 25 - Trend Filter V1 (H1 verification)
    Based on round23 (xauusd_mainline_v1.py).
    Change: suppress mean-reversion long entry when SMA(80) 5-day slope > 0
    (i.e., when long-term trend is rising, avoid counter-trend longs).
    Hypothesis: in strong uptrends, mean-reversion longs underperform; filtering
    them out should improve Sharpe without significantly reducing trade count.
    """
    ...
    def __init__(self):
        ...
        self.trend_slope_lookback = 5   # days to measure SMA(80) slope
        self.std_period = 20
        self.z_entry = 1.0
        self.atr_period = 14
    ...
    def on_marketdatafeed(self, md, ab):
        ...
        # Trend filter: if SMA(80) slope over last 5 days is positive, skip entry
        trend_sma_now = float(trend_sma.iloc[-1])
        trend_sma_prev = float(trend_sma.iloc[-1 - self.trend_slope_lookback])
        trend_slope = trend_sma_now - trend_sma_prev
        if trend_slope > 0:
            return  # suppress long entry in uptrend
