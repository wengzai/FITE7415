...
class AlgoEvent:
    """
    Round 25 - Trend Filter V2 (H1 verification, relaxed filter)
    Based on round23 (xauusd_mainline_v1.py).
    Change: suppress mean-reversion long entry when SMA(80) 10-day slope > threshold.
    Uses a longer lookback (10 days) to smooth out noise in slope measurement.
    Hypothesis: a more stable slope signal reduces false positives in the filter.
    """
    ...
    def __init__(self):
        ...
        self.trend_slope_lookback = 10  # longer lookback for smoother slope
        self.trend_slope_threshold = 1.0  # only filter if slope > $1 per day (meaningful uptrend)
        self.std_period = 20
        self.z_entry = 1.0
        self.atr_period = 14
        self.stop_atr_multiple = 1.2
        self.take_profit_atr_multiple = 3.0
        self.max_hold_days = 4
    ...
    def on_marketdatafeed(self, md, ab):
        ...
        if pd.isna(sma.iloc[-1]) or pd.isna(std.iloc[-1]) or pd.isna(atr.iloc[-1]):
            return
        if len(trend_sma.dropna()) < self.trend_slope_lookback + 1:
            return
        if pd.isna(trend_sma.iloc[-1]) or pd.isna(trend_sma.iloc[-1 - self.trend_slope_lookback]):
            return
        ...
        # Trend filter: daily slope over last 10 days; suppress if slope > $1/day
        trend_sma_now = float(trend_sma.iloc[-1])
        trend_sma_prev = float(trend_sma.iloc[-1 - self.trend_slope_lookback])
        daily_slope = (trend_sma_now - trend_sma_prev) / self.trend_slope_lookback
        if daily_slope > self.trend_slope_threshold:
            return
